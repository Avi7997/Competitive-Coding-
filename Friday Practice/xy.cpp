...
int main(){
  ...
  EACH(x, m2) {
      if(m1[x.F] != 0)
        diff += (x.S - m1[x.F] > 0)?(x.S - m1[x.F]):0;
    else
        diff+= x.S;
  }

  //cout<<"ver first difference: "<<diff<<endl;
  if(diff - m1['?'] == 0)
    ans++;
    //pm(m1);
    int arr[26];
    ...
    //pv(arr);
    //cout<<arr[ch[0]-97]<<endl;
    int pq = ch.length(), rs = s.length();
  FOR(i,pq, rs){
      m1[s[i]] ++;
      m1[s[i-ch.length()]]--;
      if(s[i] != '?'){
          diff -= (arr[s[i]-97] - m1[s[i]] >-1)?1:0;
          // diff += (arr[s[i]-97] - m1[s[i]] >-1)?1:0;
      }
      if(s[i-ch.length()] != '?'){
        diff += (arr[s[i-ch.length()]-97] - m1[s[i-ch.length()]] > 0 /*&& arr[s[i]-97]!=0*/)?1:0;
    }
    //if(i==5){cout<<"for you my love : "<<arr[s[i]-97]<<" "<<m1[s[i-ch.length()]]<<endl;}
        //pm(m1);
        //pv(arr);
    //cout<<diff<<" <- diff "<<m1['?']<<" <- ?"<<endl<<endl;
      if(diff - m1['?'] <= 0){
        ans++;
        //cout<<"in here : "<<ans<<" i :  "<<i<<" "<<s[i]<<endl<<endl;
    }
  }
  cout<<ans<<endl;
  return 0;
