...
int main(){
  ...
  //   cout<<ar[i]<<" ";
    ...
    // REP(i,n+1)
    //     cout<<v2[i]<<" ";
    int ans = 0, ulti = INT_MIN;
    //cout<<total<<endl;

    FOR(i,0,q-1)
        FOR(j,i+1,q){
            ans = total;
            //cout<<"Ans In : "<<ans<<endl;
            int x = max(pai[i].F, pai[j].F);
            int y = min(pai[i].S, pai[j].S);
            if(x<=y)
                ans -= v2[y] - v2[x-1];
            ans -= v1[pai[i].S] - v1[pai[i].F-1];
            ans -= v1[pai[j].S] - v1[pai[j].F-1];
            //cout<<pai[i].S<<" "<<pai[i].F-1<<" "<<pai[j].S<<" "<<pai[j].F-1<<endl;
            //cout<<v1[pai[i].S] - v1[pai[i].F-1]<<" "<<v1[pai[j].S] - v1[pai[j].F-1]<<endl;
